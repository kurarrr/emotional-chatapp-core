...
// https ver
var CORPUS_API_URL = 'https://tk2-255-37079.vs.sakura.ne.jp:8640/prediction_api';
...
FriendlyChat.prototype.loadMessages = function() {
  ...
  var setMessage = function(data) {
    ...
    if(val.effect){
      ...
      // if(val.effect[userAttr]){
      //   effectCss = val.effect[userAttr];
      // }
      // 初期仕様 : native -> non-nativeのmessageのみフォント表示
      // 逆もそうだったが, 全部フォント表示になった
      if(val.effect['native']){
        effectCss = val.effect['native'];
      }else if(val.effect['non-native']){
        effectCss = val.effect['non-native'];
      }
    }
    var valence,arousal;
    valence = arousal = 0.5;
  ...
  if (this.messageInput.value && this.checkSignedInWithMessage()) {
    ...
    this.messagesRef.push({
      ...
      $.ajax({
        ...
        console.log(String(endTime - startTime) + "msec");
        var opUserAttr = "";
        ...
        for(var idx = 0; idx < self.vaVal.length; idx++){
          // var temp = (cor*dat['Valence'] - self.vaVal[idx]['Valence']) * (cor*dat['Valence'] - self.vaVal[idx]['Valence'])
          //           + (cor*dat['Arousal'] - self.vaVal[idx]['Arousal']) * (cor*dat['Arousal'] - self.vaVal[idx]['Arousal']);
          var temp = Math.abs(dat['Valence'] - self.vaVal[idx]['Valence'])
          if(temp<distmin){
            distmin = temp;
            argmin = idx;
          }
        ...
        console.log(distmin);
        console.log(self.vaVal[argmin]['name']);
        var msgEffect = {};
        msgEffect[opUserAttr] = 'font-' + self.vaVal[argmin]['name'].replace('.','-');
        var updates = {
          effect : msgEffect,
          Valence : dat['Valence'],
          Arousal : dat['Arousal']
...
FriendlyChat.prototype.bindUserAttr = function(){
  // jQuery object
  var $select = $("#user-attr-select");
  this.userAttr = $select.val();
  var self = this;
  $select.change(function(e){
      // user-attr変更の処理
      // このスコープでthisはjQuery object を指すので注意
      self.userAttr = $select.val();
      self.clearEffect();
      self.loadMessages();
  });
};

FriendlyChat.prototype.clearEffect = function(){
